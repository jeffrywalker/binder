...
_python_version_ = "{}.{}".format(
    sys.version_info.major, sys.version_info.minor
)  # should be formatted: 2.7, 3.5, 3.6, ...
...
def get_compiler_family():
    ...
    if "clang" in Options.compiler:
        return "clang"
    if "gcc" in Options.compiler:
        return "gcc"
    if "cl" in Options.compiler:
        return "cl"
    return "unknown"
...
def get_cmake_compiler_options():
    ...
    if Platform == "linux" and Options.compiler == "clang":
        return " -DCMAKE_C_COMPILER=`which clang` -DCMAKE_CXX_COMPILER=`which clang++`"
    if Platform == "linux" and Options.compiler == "gcc":
        return " -DCMAKE_C_COMPILER=`which gcc` -DCMAKE_CXX_COMPILER=`which g++`"
...
def install_llvm_tool(
    ...
    """
    if not os.path.isdir(prefix_root):
    ...
    llvm_version, headers = (
        ("13.0.0", "tools/clang/lib/Headers/clang-resource-headers clang")
        if Platform == "macos"
        and platform.machine() == "arm64"
        or Platform == "windows"
        else ("6.0.1", "tools/clang/lib/Headers/clang-headers")
    )
    ...
    res, output = execute(
        "Getting binder HEAD commit SHA1...",
        "cd {} && git rev-parse HEAD".format(source_location),
        return_="tuple",
        silent=True,
    )
    if res:
        ...
        binder_head = output.split("\n")[0]

    signature = dict(
        config="LLVM install by install_llvm_tool version: 1.5.1, HTTPS",
        binder=binder_head,
        llvm_version=llvm_version,
        compiler=compiler,
        gcc_install_prefix=gcc_install_prefix,
    )
    ...
    if os.path.isfile(signature_file_name):
        with open(signature_file_name) as f:
            disk_signature = json.load(f)

    if signature == disk_signature:
        print(
            "LLVM:{} + Binder install is detected at {}, skipping LLVM installation and Binder building procedures...\n".format(
                llvm_version, build_dir
            )
        )

    else:
        print(
            "LLVM build detected, but config/binder version has changed, performing a clean rebuild..."
        )
        ...
        llvm_url, clang_url = {
            "6.0.1": (
                ...
                "https://releases.llvm.org/6.0.1/cfe-6.0.1.src.tar.xz",
            ),
            "13.0.0": (
        ...
        if not os.path.isfile(prefix + "/CMakeLists.txt"):
            ...
            execute(
                ...
                "cd {prefix_root} && mkdir llvm-{llvm_version}.src && curl -LJ {llvm_url} | tar --strip-components=1 -Jxom -C llvm-{llvm_version}.src && mv llvm-{llvm_version}.src {prefix}".format(
                    **locals()
                ),
            )
        ...
        if not os.path.isdir(clang_path):
            ...
            if Platform == "windows":
                # avoid hardcoding nuance that 6.0.1 is cfe-* not clang-*
                clangPackageName = os.path.basename(clang_url)
                clangPackageName = str.replace(clangPackageName, ".src.tar.xz", "")
                # NOTE: extracting the *.tar.xz on windows throws errors with the symlinks, use 7za instead
                execute(
                    "Download Clang source...",
                    "cd {prefix_root} && mkdir clang-{llvm_version}.src && curl -LJ {clang_url} --output clang-{llvm_version}.src.tar.xz && 7za x -so clang-{llvm_version}.src.tar.xz | 7za x -si -ttar && mv {clangPackageName}.src {clang_path}".format(
                        **locals()
                    ),
                )
            else:
                execute(
                    "Download Clang source...",
                    "cd {prefix_root} && mkdir clang-{llvm_version}.src && curl -LJ {clang_url} | tar --strip-components=1 -Jxom -C clang-{llvm_version}.src && mv clang-{llvm_version}.src {clang_path}".format(
                        **locals()
                    ),
                )

        if not os.path.isdir(prefix + "/tools/clang/tools/extra"):
            os.makedirs(prefix + "/tools/clang/tools/extra")
        ...
        tool_link_path = "{prefix}/tools/clang/tools/extra/{name}".format(
        ...
        if not os.path.isfile(cmake_lists):
            with open(cmake_lists, "w") as f:
                f.write(tool_build_line + "\n")

        config = "-DCMAKE_BUILD_TYPE={build_type}".format(
        ...
        if Platform == "windows":
            # fmt: off
            cmdStr = "{vcvars} && set CC=cl && set CXX=cl && " + \
                "cd {build_dir} && " + \
                "cmake -G Ninja {config} -DLLVM_ENABLE_EH=1 -DLLVM_ENABLE_RTTI=ON .. && " + \
                "ninja binder {headers} {jobs}"
            # fmt: on
            execute(
                "Preparing Windows build environment to build: {}...".format(name),
                cmdStr.format(
                    vcvars=Options.vcvarsbat,
                    build_dir=build_dir,
                    config=config,
    ...
    if not os.path.isfile(executable):
        print(
            "\nEncounter error while running install_llvm_tool: Build is complete but executable {} is not there!!!".format(
                executable
            )
        )
        sys.exit(1)
...
def install_pybind11(prefix, clean=True):
    ...
    git_checkout = (
        "( git fetch && git reset --hard {0} )".format(_pybind11_version_)
        if clean
        else "git checkout {}".format(_pybind11_version_)
    )
    ...
    if not os.path.isdir(package_dir):
        execute(
            "Clonning pybind11...",
            "cd {} && git clone https://github.com/RosettaCommons/pybind11.git".format(
                prefix
            ),
        )
    execute(
        "Checking out PyBind11 revision: {}...".format(_pybind11_version_),
        "cd {package_dir} && ( {git_checkout} )".format(
            package_dir=package_dir, git_checkout=git_checkout
        ),
        silent=True,
    )
    print()
    ...
    if not os.path.isfile(include):
        print(
            "\nEncounter error while running install_pybind11: Install is complete but include file {} is not there!!!".format(
                include
            )
        )
        sys.exit(1)
...
def main(args):
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=int,
        help="Number of processors to use on when building, use '-j' with no arguments to launch job-per-core. (default: 1) ",
    )
    parser.add_argument(
        "--type",
        default="Release",
    ...
    parser.add_argument(
        "--compiler",
        default="cl" if Platform == "windows" else "clang",
        help="Compiler to use, default is clang",
    )
    parser.add_argument(
        "--vcvarsbat",
        default=r'"C:\Program Files (x86)\Microsoft Visual Studio\2019\Professional\VC\Auxiliary\Build\vcvars64.bat"',
        help="*.bat file used to setup windows environment prior to invoking CMake",
    )
    parser.add_argument(
        "--binder",
        default="",
        help='Path to Binder tool. If none is given then download, build and install binder into build/ directory. Use "--binder-debug" to control which mode of binder (debug/release) is used.',
    )
    parser.add_argument(
        "--binder-debug",
        action="store_true",
        help="Run binder tool in debug mode (only relevant if no '--binder' option was specified)",
    )
    parser.add_argument("--pybind11", default="", help="Path to pybind11 source tree")
    parser.add_argument(
        "--annotate-includes",
        action="store_true",
        help="Annotate includes in generated source files",
    )
    parser.add_argument(
        "--trace",
        action="store_true",
        help="Binder will add trace output to to generated source files",
    )
    ...
    if not Options.binder:
        Options.binder = install_llvm_tool(
            "binder",
            source_path + "/source",
            source_path + "/build",
            debug=Options.binder_debug,
            compiler=Options.compiler,
            jobs=Options.jobs,
            gcc_install_prefix=None,
        )

    if not Options.pybind11:
        Options.pybind11 = install_pybind11(source_path + "/build")

    print(
        "Binder binaries now ready at: {}\nPybind11 clone is ready at: {}".format(
            Options.binder, Options.pybind11
        )
    )
